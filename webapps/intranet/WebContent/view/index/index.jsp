<script type="text/javascript" src="view/index/js/toolbar.js"></script>
<script type="text/javascript" src="view/repository/js/repo_grid.js"></script>
<script type="text/javascript" src="view/repository/js/repo_win.js"></script>
<jsp:include page="../correos/include.jsp"></jsp:include>
<jsp:include page="../usuarios/include.jsp"></jsp:include>
<jsp:include page="../eventos/include.jsp"></jsp:include>
<div id="toolbar"></div>
<!--
<div style="width: 100%; height: 100%;" id="container">
	<div style="border-top: 1px solid #ff9999;width: 20%;height:100%;float:left;"></div>
	<div style="border-top: 1px solid #ff9999;width: 80%;height:100%;float:left;"></div>
</div>
-->
<div style="display: none;"><div id="grid_repositorios"></div></div>

<table style="height: 100%; width: 100%;">
	<tr>
		<td width="20%"><div id="mis_bandejas"></div></td>
		<td width="60%"><div id="correos"></div></td>
		<td width="20%"><div id="mis_eventos"></div></td>
	</tr>
</table>
