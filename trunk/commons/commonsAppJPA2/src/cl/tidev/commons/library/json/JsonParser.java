...
public class JsonParser {
	...
	public static String parseClass(Object obj){
				...
				if( field.get(obj) != null ){
					if( !JpaUtils.isToManyAnnotation(field) ){ 
						jsonData.add( new StringBuilder()
							.append( JsonUtils.parse( field.getName()) ).append(":")
							.append( parseValue(field, obj) ).toString() );
					}
				} else {
					jsonData.add( new StringBuilder()
						.append( JsonUtils.parse( field.getName()) ).append(":")
		...
		return new StringBuilder().append("{")
			.append( ArrayUtils.implode(jsonData, ", ")).append("}").toString();
	}
	...
	private static String parseValue(Class<?> dataType, Object value){
		...
		if( value == null){
			...
			if( value.getClass().isInstance( value ) )  parsedValue = parseClass( value);
	...
	public static String parseMap(Map<?, ?> map){
		...
		while( keysIterator.hasNext() ){
			...
			if(map.get(key) != null){
				jsonData.add( new StringBuilder().append( JsonUtils.parse(key) ).append(":")
